...
def main():
    ...
    print("Tabela ordenada por pontos:")
    ...
    print("Tabela ordenada por d. golos:")
    tab = sorted(tabela, key=lambda x: x[4]-x[5], reverse=True )
    printTabela(tab)
    ...
    tab = sorted(tabela, key= lambda x : (pontos(x) , x[4]-x[5]), reverse=True )
    printTabela(tab)
